var legalcareDutyColShape = mp.colshapes.newSphere(-855.0792, -10.6365, -85.4408, 2, 0);
...
mp.events.add("server:Keybind:KeyE", (player) => {
  if (mp.players.exists(player)) {
    ...
    if (businessData.playerBusinessDuty === "Y" && businessData.businessName === "Legal Care") {
        /*if(legalcareVehColShape.isPointWithin(player.position)) {
            //player.call("client:legalcare:openMenu");
        }*/
    }
    if(businessData.businessName === "Legal Care") {
        if(legalcareDutyColShape.isPointWithin(player.position)) {
            player.call("client:legalcare:openDutyMenu");
        }
    }
  }
});
...
mp.events.add("server:Keybind:KeyO", (player) => {
    if(mp.players.exists(player)) {
      ...
      if (businessData.playerBusinessDuty == "Y" && businessData.businessName == "Legal Care") {
          gm.databaseManager.getConnection().query("SELECT r.businessRankName FROM business f LEFT JOIN businessranks r ON f.businessID = r.businessID WHERE f.businessID = 15", function (errUp, resUp) {
              ...
              if (resUp.length > 0) {
                  ...
                  resUp.forEach(function(rank) {
                      ...
                      if (c == resUp.length) {
                          player.call("client:legalcare:openInteractionMenu", [businessData.canInvite, JSON.stringify(ranks)]);
                      }
                      c++;
                  });
              }
...
function hirePlayer(player, rank) {
    if(mp.players.exists(player)) {
      ...
      if(mp.players.exists(currentTarget)) {
        if(currentTarget){
            gm.databaseManager.getConnection().query("SELECT id FROM businessranks WHERE businessID = 15 AND businessRankName = ?", [rank], function (err1, res1) {
                if (err1) console.log("Error in legalcare Hire Player Query1: "+err1);
                if (res1.length == 1) {
                    res1.forEach(function(rankID) {
                        var id = rankID.id;
                        var targetId = parseInt(currentTarget.data.internalId);
                        gm.databaseManager.getConnection().query("INSERT INTO businessusers(playerCharID,businessID,businessRankID,playerBusinessDuty) VALUES(?,15,?,'N')", [targetId, id], function(err2, res2) {
                            if (err2) console.log("Error in legalcare Hire Player Query2: "+err2);
                            else {
                                player.notify("Die Person wurde erfolgreich eingestellt!");
                                currentTarget.notify("Du wurdest Legal Care als "+rank+" eingestellt!");
...
mp.events.add("server:legalcare:hirePlayer", hirePlayer);
    ...
    if(mp.players.exists(player)) {
      ...
      if(mp.players.exists(currentTarget)) {
        if(currentTarget){
            ...
            gm.databaseManager.getConnection().query("DELETE FROM businessusers WHERE businessID = 15 AND playerCharID = ?", [targetId], function (err1, res1) {
                if (err1) console.log("Error in legalcare Fire Player Query1: "+err1);
                else {
                    player.notify("Die Person wurde erfolgreich gefeuert!");
                    currentTarget.notify("Du wurdest von Legal Care entlassen!");
                    gm.databaseManager.getConnection().query("SELECT f.businessName, f.businessID, r.id AS businessRankID, r.businessRankName, r.businessRank, r.canBill, r.canInvite, r.payCheck, u.playerBusinessDuty FROM businessusers u LEFT JOIN businessranks r ON r.id = u.businessRankID LEFT JOIN business f ON f.businessID = u.businessID WHERE u.playerCharId = ?", [targetId], function (err2, res2) {
                        if (err2) console.log("Error on Set Fraction");
...
mp.events.add("server:legalcare:firePlayer", firePlayer);

mp.events.add("server:legalcare:onDuty", (player) => {
    if(mp.players.exists(player)) {
      var i = 0;
...
mp.events.add("server:legalcare:offDuty", (player) => {
    if(mp.players.exists(player)) {
