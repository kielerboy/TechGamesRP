...
let getCharIdByCommandName = function (name, clb) {
    gm.databaseManager.getConnection().query("SELECT id FROM characters WHERE commandName = ?", [name], function (err, res) {
...
mp.events.addCommand("tp", (player, location) => {
  ...
  if(fractionData.fractionName == "NOOSE") {
        if (location == "medical_center") {
            ...
            player.spawn(new mp.Vector3(165.4010000, -984.36895751, 30.09192276000));
            ...
            player.spawn(new mp.Vector3(-1421.015, -3012.587, -80.000));
...
if(fractionData.fractionName == "NOOSE"){
      ...
      gm.databaseManager.getConnection().query('UPDATE characters SET drink = ?, food = ? WHERE id = ?', [valueDrink, valueFood, player.data.internalId], function (err6, res2, row2) {
...
if(fractionData.fractionName == "NOOSE"){
      ...
      gm.databaseManager.getConnection().query('UPDATE characters SET health = ? WHERE id = ?', [valueHealth, player.data.internalId], function (err7, res7, row7) {
...
mp.events.addCommand("pos", (player, text) => {
  console.log(text + ": " + player.position.x + ", " + player.position.y + ", " + player.position.z + ", " + player.heading);
