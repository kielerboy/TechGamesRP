var bahamaDutyColShape = mp.colshapes.newSphere(-1388.0180, -607.3753, 30.3195, 2, 0);
...
mp.events.add("server:Keybind:KeyE", (player) => {
  if (mp.players.exists(player)) {
    ...
    if (businessData.businessName === "Bahama Mamas") {
        if(bahamaDutyColShape.isPointWithin(player.position)) {
            player.call("client:bahama:openDutyMenu");
        }
    }
  }
});
...
mp.events.add("server:Keybind:KeyO", (player) => {
  if(mp.players.exists(player)) {
    ...
    if (businessData.playerBusinessDuty == "Y" && businessData.businessName == "Bahama Mamas") {
        gm.databaseManager.getConnection().query("SELECT r.businessRankName FROM business f LEFT JOIN businessRanks r ON f.businessID = r.businessID WHERE f.businessID = 4", function (errUp, resUp) {
            ...
            if (resUp.length > 0) {
                ...
                resUp.forEach(function(rank) {
                    ...
                    if (c == resUp.length) {
                        player.call("client:bahama:openInteractionMenu", [businessData.canInvite, JSON.stringify(ranks)]);
                    }
                    c++;
                });
            }
...
function hirePlayer(player, rank) {
  if (mp.players.exists(player)) {
    ...
    if(mp.players.exists(currentTarget)) {
      if(currentTarget){
          gm.databaseManager.getConnection().query("SELECT id FROM businessRanks WHERE businessID = 4 AND businessRankName = ?", [rank], function (err1, res1) {
              if (err1) console.log("Error in Bahama Hire Player Query1: "+err1);
              if (res1.length == 1) {
                  res1.forEach(function(rankID) {
                      var id = rankID.id;
                      var targetId = parseInt(currentTarget.data.internalId);
                      gm.databaseManager.getConnection().query("INSERT INTO businessUsers(playerCharID,businessID,businessRankID,playerBusinessDuty) VALUES(?,4,?,'N')", [targetId, id], function(err2, res2) {
                          if (err2) console.log("Error in Bahama Hire Player Query2: "+err2);
                          else {
                              player.notify("Die Person wurde erfolgreich eingestellt!");
                              currentTarget.notify("Du wurdest beim Bahama Mamas als "+rank+" eingestellt!");
...
mp.events.add("server:bahama:hirePlayer", hirePlayer);
  ...
  if (mp.players.exists(player)) {
    ...
    if (mp.players.exists(currentTarget)) {
      if(currentTarget){
          ...
          gm.databaseManager.getConnection().query("DELETE FROM businessUsers WHERE businessID = 4 AND playerCharID = ?", [targetId], function (err1, res1) {
              if (err1) console.log("Error in Bahama Fire Player Query1: "+err1);
              else {
                  player.notify("Die Person wurde erfolgreich gefeuert!");
                  currentTarget.notify("Du wurdest aus dem Bahama Mamas entlassen!");
                  gm.databaseManager.getConnection().query("SELECT f.businessName, f.businessID, r.id AS businessRankID, r.businessRankName, r.businessRank, r.canBill, r.canInvite, r.payCheck, u.playerBusinessDuty FROM businessUsers u LEFT JOIN businessRanks r ON r.id = u.businessRankID LEFT JOIN business f ON f.businessID = u.businessID WHERE u.playerCharId = ?", [targetId], function (err2, res2) {
                      if (err2) console.log("Error on Set Fraction");
...
mp.events.add("server:bahama:firePlayer", firePlayer);



mp.events.add("server:bahama:onDuty", (player) => {
  if(mp.players.exists(player)) {
    ...
    gm.databaseManager.getConnection().query("UPDATE businessUsers SET playerBusinessDuty = 'Y' WHERE playerCharID = " + player.data.internalId, function (errUp, resUp) {
      ...
      gm.databaseManager.getConnection().query("SELECT f.businessName, f.businessID, r.id AS businessRankID, r.businessRankName, r.businessRank, r.canBill, r.canInvite, r.payCheck, u.playerBusinessDuty FROM businessUsers u LEFT JOIN businessRanks r ON r.id = u.businessRankID LEFT JOIN business f ON f.businessID = u.businessID WHERE u.playerCharID = ?", [player.data.internalId], function (err2, res2) {
        if (err2) console.log("Error on Set Business after Bahama onDuty");
...
mp.events.add("server:bahama:offDuty", (player) => {
  if(mp.players.exists(player)) {
    ...
    gm.databaseManager.getConnection().query("SELECT appearance, data FROM characterModel WHERE internalId = ?", [player.data.internalId], function (err2, res2) {
        if (err2) console.log("Error in setModel + Clothes on Bahama offDuty");
        ...
        if (res2.length > 0) {
            res2.forEach(function (modelData) {
                ...
                gm.databaseManager.getConnection().query("UPDATE businessUsers SET playerBusinessDuty = 'N' WHERE playerCharID = " + player.data.internalId, function (errUp, resUp) {
                    ...
                    gm.databaseManager.getConnection().query("SELECT f.businessName, f.businessID, r.id AS businessRankID, r.businessRankName, r.businessRank, r.canBill, r.canInvite, r.payCheck, u.playerBusinessDuty FROM businessUsers u LEFT JOIN businessRanks r ON r.id = u.businessRankID LEFT JOIN business f ON f.businessID = u.businessID WHERE u.playerCharID = ?", [player.data.internalId], function (err2, res2) {
                        if (err2) console.log("Error on Set Fraction after Bahama offDuty");
