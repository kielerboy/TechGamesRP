...
mp.events.add("menuClick", (player, trigger, button) => {
    if (trigger === "fleecaNormal") {
        gm.databaseManager.getConnection().query("SELECT id FROM bank_konten WHERE ownerId = ?", [player.data.internalId], function (err, res) {
            ...
            if (!res.length > 0) {
                gm.databaseManager.getConnection().query("INSERT INTO bank_konten (ownerId , amout, kontonummer, beschreibung) VALUES (?,'2500',?,'Girokonto')", [player.data.internalId, kontonummer], function (err2, res2) {
                    if (err2) console.log(err2);
