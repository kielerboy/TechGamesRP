            ...
            if (itemData !== null) {
                ...
                if (overlay !== null) {
                    if (mp.players.exists(currentTarget)) {
                        ...
                        if (obj !== null && obj !== "") {
                            player.call("client:tattoo:checkMenu");
                        }
...
mp.events.add("server:tattoo:save", (player) => {
    if (mp.players.exists(player)) {
        ...
        if (mp.players.exists(currentTarget)) {
            gm.databaseManager.getConnection().query("SELECT id , amout FROM bank_konten WHERE ownerId = ?", [currentTarget.data.internalId], function(err1, res1) {
                ...
                if (res1.length > 0) {
                    res1.forEach(function(am) {
                        ...
                        if (parseFloat(tattooToAdd.price) <= parseFloat(am.amout)) {
                            var newTattooAm = parseFloat(parseFloat(am.amout) - parseFloat(tattooToAdd.price)).toFixed(2);
                            gm.databaseManager.getConnection().query("UPDATE bank_konten SET amout = ? WHERE ownerId = ?", [newTattooAm, currentTarget.data.internalId], function(err3, res3) {
                            ...
                            player.call(`notification`, ["4", "Du brauchst mehr Geld auf dem Konto"]);
                            var currentTattoos = player.getVariable("tattoos");
                            currentTattoos = JSON.parse(currentTattoos);
                            player.clearDecorations();
