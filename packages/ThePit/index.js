var thepitDutyColShape = mp.colshapes.newSphere(-1152.257, -1423.68, 4.95, 2, 0);
var thepitTattooColShape = mp.colshapes.newSphere(-1155.281, -1427.715, 4.95, 2, 0);
...
mp.events.add("server:Keybind:KeyE", (player) => {
  if (mp.players.exists(player)) {
    ...
    if (businessData.businessName === "The Pit") {
        if(thepitDutyColShape.isPointWithin(player.position)) {
            player.call("client:thepit:openDutyMenu");
        }
      if (businessData.businessName === "The Pit") {
        if(thepitTattooColShape.isPointWithin(player.position)) {
...
mp.events.add("server:Keybind:KeyO", (player) => {
  if(mp.players.exists(player)) {
    ...
    if (businessData.playerBusinessDuty == "Y" && businessData.businessName == "The Pit") {
        gm.databaseManager.getConnection().query("SELECT r.businessRankName FROM business f LEFT JOIN businessRanks r ON f.businessID = r.businessID WHERE f.businessID = 10", function (errUp, resUp) {
            ...
            if (resUp.length > 0) {
                ...
                resUp.forEach(function(rank) {
                    ...
                    if (c == resUp.length) {
                        player.call("client:thepit:openInteractionMenu", [businessData.canInvite, JSON.stringify(ranks)]);
                    }
                    c++;
                });
            }
...
function hirePlayer(player, rank) {
  if (mp.players.exists(player)) {
    ...
    if(mp.players.exists(currentTarget)) {
      if(currentTarget){
          gm.databaseManager.getConnection().query("SELECT id FROM businessRanks WHERE businessID = 10 AND businessRankName = ?", [rank], function (err1, res1) {
              if (err1) console.log("Error in thepit Hire Player Query1: "+err1);
              if (res1.length == 1) {
                  res1.forEach(function(rankID) {
                      var id = rankID.id;
                      var targetId = parseInt(currentTarget.data.internalId);
                      gm.databaseManager.getConnection().query("INSERT INTO businessUsers(playerCharID,businessID,businessRankID,playerBusinessDuty) VALUES(?,10,?,'N')", [targetId, id], function(err2, res2) {
                          if (err2) console.log("Error in thepit Tattoo Hire Player Query2: "+err2);
                          else {
                              player.notify("Die Person wurde erfolgreich eingestellt!");
                              currentTarget.notify("Du wurdest beim The Pit als "+rank+" eingestellt!");
...
mp.events.add("server:thepit:hirePlayer", hirePlayer);
  ...
  if (mp.players.exists(player)) {
    ...
    if (mp.players.exists(currentTarget)) {
      if(currentTarget){
          ...
          gm.databaseManager.getConnection().query("DELETE FROM businessUsers WHERE businessID = 10 AND playerCharID = ?", [targetId], function (err1, res1) {
              if (err1) console.log("Error in thepit Tattoo Fire Player Query1: "+err1);
              else {
                  player.notify("Die Person wurde erfolgreich gefeuert!");
                  currentTarget.notify("Du wurdest aus dem thepit Tattoo entlassen!");
                  gm.databaseManager.getConnection().query("SELECT f.businessName, f.businessID, r.id AS businessRankID, r.businessRankName, r.businessRank, r.canBill, r.canInvite, r.payCheck, u.playerBusinessDuty FROM businessUsers u LEFT JOIN businessRanks r ON r.id = u.businessRankID LEFT JOIN business f ON f.businessID = u.businessID WHERE u.playerCharId = ?", [targetId], function (err2, res2) {
                      if (err2) console.log("Error on Set Fraction");
  ...
  if(mp.players.exists(player)) {
    ...
    gm.databaseManager.getConnection().query("UPDATE businessUsers SET playerBusinessDuty = 'Y' WHERE playerCharID = " + player.data.internalId, function (errUp, resUp) {
      ...
      gm.databaseManager.getConnection().query("SELECT f.businessName, f.businessID, r.id AS businessRankID, r.businessRankName, r.businessRank, r.canBill, r.canInvite, r.payCheck, u.playerBusinessDuty FROM businessUsers u LEFT JOIN businessRanks r ON r.id = u.businessRankID LEFT JOIN business f ON f.businessID = u.businessID WHERE u.playerCharID = ?", [player.data.internalId], function (err2, res2) {
        if (err2) console.log("Error on Set Business after thepit onDuty");
...
mp.events.add("server:thepit:offDuty", (player) => {
  if(mp.players.exists(player)) {
    ...
    gm.databaseManager.getConnection().query("SELECT appearance, data FROM characterModel WHERE internalId = ?", [player.data.internalId], function (err2, res2) {
        if (err2) console.log("Error in setModel + Clothes on thepit offDuty");
