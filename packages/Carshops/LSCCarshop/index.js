let LsccardealerShape = mp.colshapes.newSphere(-350.57, -116.96, 38.93, 3, 0);
let LsccardealerSpawn = new mp.Vector3(-355.77, -115.25, 38.69);
...
mp.events.add("server:Keybind:KeyE", (player) => {
  if(mp.players.exists(player)) {
    ...
    if (businessData.businessName == "Los Santos Customs") {
        if(LsccardealerShape.isPointWithin(player.position)) {
            player.call("client:LscCarShop:OpenMenu");
        }
    }
  }
});
...
function playerExitColshapeHandler(player, shape) {
  if (mp.players.exists(player)) {
    if(shape == LsccardealerShape) {
      //Spieler hat colshape verlassen
      player.call("client:LscCarShop:closeMenu")
    }
  }
}
mp.events.add("playerExitColshape", playerExitColshapeHandler);

function lscgiveCar(player, model, amout) {
  if (mp.players.exists(player)) {
          ...
          if ( (amout*1) > (money*1)){
            ...
            gm.databaseManager.getConnection().query("UPDATE characters SET money = ? WHERE id = ?", [newAm, player.data.internalId], function (err3, res3) {
                ...
                gm.databaseManager.getConnection().query("SELECT * FROM vehicles", function (err, res) {
                  ...
                  var randomOK = true;
                  do{
                    randomNumPlate = "LSC" + Math.floor(Math.random() * 99999);
                    randomOK = true;
                    res.forEach(toCheck => {
                      if(toCheck.numberplate == randomNumPlate) randomOK = false;
                    });
                  } while(!randomOK);
                  var veh = mp.vehicles.new(model, LsccardealerSpawn, {});
                  ...
                  gm.databaseManager.getConnection().query("INSERT INTO vehicles (garage, isSpawned, modelId, type, numberplate, owner, fuel, odometer, tuning, isImpounded, fraction, buyDate) VALUES (22, 'Y', ?, 1,?, ?, 100, 100, ?, 'N','LSC', ?)", [model, randomNumPlate, player.data.internalId, JSON.stringify(vehTuning), buyDate], function (err, res) {
                    if (err) console.log(err);
...
mp.events.add("server:LscCarShop:giveCar", lscgiveCar);
