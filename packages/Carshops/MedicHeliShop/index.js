let MedichelidealerShape = mp.colshapes.newSphere(-456.3433, -307.75, 78.16822, 3, 0);
let MedichelidealerSpawn = new mp.Vector3(-452.0834, -300.3148, 78.1683);

mp.events.add("server:Keybind:KeyE", (player) => {
  if (mp.players.exists(player)) {
    if(MedichelidealerShape.isPointWithin(player.position)) {
      var fractionData = player.data.fractionData;
      fractionData = JSON.parse(fractionData);
      if (fractionData.fractionName == "LSMC") {
        player.call("client:MedicHeliShop:OpenMenu");
      }
    }
  }
});
...
function playerExitColshapeHandler(player, shape) {
  if (mp.players.exists(player)) {
    if(shape == MedichelidealerShape) {
      //Spieler hat colshape verlassen
      player.call("client:MedicHeliShop:closeMenu")
    }
  }
}
mp.events.add("playerExitColshape", playerExitColshapeHandler);

function givemedicheli(player, model, amout) {
  if (mp.players.exists(player)) {
          ...
          if (amout > am.amout) {
            ...
            gm.databaseManager.getConnection().query("UPDATE bank_konten SET amout = ? WHERE ownerId = ?", [newAm, player.data.internalId], function (err3, res3) {
                ...
                gm.databaseManager.getConnection().query("SELECT * FROM vehicles", function (err, res) {
                  ...
                  var veh = mp.vehicles.new(model, MedichelidealerSpawn, {});
                  ...
                  gm.databaseManager.getConnection().query("INSERT INTO vehicles (garage, isSpawned, modelId, type, numberplate, owner, fuel, odometer, tuning, isImpounded, fraction, buyDate) VALUES (28, 'Y', ?, 1, ?, ?, 100, 100, ?, 'N', 'LSMC', ?)", [model, randomNumPlate, player.data.internalId, JSON.stringify(vehTuning), buyDate], function (err, res) {
                    if (err) console.log(err);
...
mp.events.add("server:MedicHeliShop:giveCar", givemedicheli);
