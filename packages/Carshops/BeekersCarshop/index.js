let BeekerscardealerShape = mp.colshapes.newSphere(133.2848, 6630.3110, 31.6840, 3, 0);
let BeekerscardealerSpawn = new mp.Vector3(141.6, 6637.630, 31.6878);
...
mp.events.add("server:Keybind:KeyE", (player) => {
  if(mp.players.exists(player)) {
    ...
    if (businessData.businessName == "Beekers Garage & Part") {
        if(BeekerscardealerShape.isPointWithin(player.position)) {
            player.call("client:beekersCarShop:OpenMenu");
        }
    }
  }
});
...
function playerExitColshapeHandler(player, shape) {
  if (mp.players.exists(player)) {
    if(shape == BeekerscardealerShape) {
      //Spieler hat colshape verlassen
      player.call("client:beekersCarShop:closeMenu")
    }
  }
}
mp.events.add("playerExitColshape", playerExitColshapeHandler);

function beekersgiveCar(player, model, amout) {
  if (mp.players.exists(player)) {
          ...
          if ( (amout*1) > (money*1)){
            ...
            gm.databaseManager.getConnection().query("UPDATE characters SET money = ? WHERE id = ?", [newAm, player.data.internalId], function (err3, res3) {
                ...
                gm.databaseManager.getConnection().query("SELECT * FROM vehicles", function (err, res) {
                  ...
                  var randomOK = true;
                  do{
                    randomNumPlate = "BE" + Math.floor(Math.random() * 99999);
                    randomOK = true;
                    res.forEach(toCheck => {
                      if(toCheck.numberplate == randomNumPlate) randomOK = false;
                    });
                  } while(!randomOK);
                  var veh = mp.vehicles.new(model, BeekerscardealerSpawn, {});
                  ...
                  gm.databaseManager.getConnection().query("INSERT INTO vehicles (garage, isSpawned, modelId, type, numberplate, owner, fuel, odometer, tuning, isImpounded, fraction, buyDate) VALUES (24, 'Y', ?, 1,?, ?, 100, 100, ?, 'N','Beekers', ?)", [model, randomNumPlate, player.data.internalId, JSON.stringify(vehTuning), buyDate], function (err, res) {
                    if (err) console.log(err);
...
mp.events.add("server:beekersCarShop:giveCar", beekersgiveCar);
