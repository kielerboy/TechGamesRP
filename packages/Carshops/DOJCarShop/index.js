let DOJcardealerShape = mp.colshapes.newSphere(261.849, -336.92, 45.01, 3, 0);
let DOJcardealerSpawn = new mp.Vector3(279.174, -336.798, 44.91);

mp.events.add("server:Keybind:KeyE", (player) => {
  if (mp.players.exists(player)) {
    if(DOJcardealerShape.isPointWithin(player.position)) {
      ...
      if (fractionData.fractionName == "Department of Justice") {
        player.call("client:DOJCarShop:OpenMenu");
      }
    }
  }
});
...
function playerExitColshapeHandler(player, shape) {
  if(mp.players.exists(player)) {
    if(shape == DOJcardealerShape) {
      //Spieler hat colshape verlassen
      player.call("client:DOJCarShop:closeMenu")
    }
  }
}
mp.events.add("playerExitColshape", playerExitColshapeHandler);

function givedojCar(player, model, amout) {
  if(mp.players.exists(player)) {
          ...
          if (amout > am.amout) {
            ...
            gm.databaseManager.getConnection().query("UPDATE bank_konten SET amout = ? WHERE ownerId = ?", [newAm, player.data.internalId], function (err3, res3) {
                ...
                gm.databaseManager.getConnection().query("SELECT * FROM vehicles", function (err, res) {
                  ...
                  var randomOK = true;
                  do{
                    randomNumPlate = "DOJ-" + Math.floor(Math.random() * 1000);
                    randomOK = true;
                    res.forEach(toCheck => {
                      if(toCheck.numberplate == randomNumPlate) randomOK = false;
                    });
                  } while(!randomOK);
                  var veh = mp.vehicles.new(model, DOJcardealerSpawn, {});
                  ...
                  gm.databaseManager.getConnection().query("INSERT INTO vehicles (garage, isSpawned, modelId, type, numberplate, owner, fuel, odometer, tuning, isImpounded, fraction, buyDate) VALUES (21, 'Y', ?, 1, ?, ?, 100, 100, ?, 'N', 'DOJ')", [model, randomNumPlate, player.data.internalId, JSON.stringify(vehTuning), buyDate], function (err, res) {
                    if (err) console.log(err);
...
mp.events.add("server:DOJCarShop:giveCar", givedojCar);
