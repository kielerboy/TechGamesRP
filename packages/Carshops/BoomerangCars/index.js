function boomeranggiveCar(player, model, amout) {
      ...
      if (res.length > 0) {
          res.forEach(function (am) {
            if (amout > am.amout) {
              ...
              console.log("BoomerangCars Test - nicht genug Geld");
            } else {
              ...
              gm.databaseManager.getConnection().query("UPDATE bank_konten SET amout = ? WHERE ownerId = ?", [newAm, player.data.internalId], function (err3, res3) {
                  ...
                  gm.databaseManager.getConnection().query("SELECT * FROM vehicles", function (err, res) {
                    ...
                    gm.databaseManager.getConnection().query("INSERT INTO vehicles (garage, isSpawned, modelId, type, numberplate, owner, fuel, odometer, tuning, isImpounded, buyDate) VALUES (29, 'N', ?, 1, ?, ?, 100, 100, ?, 'N', ?)", [model, randomNumPlate, player.data.internalId, JSON.stringify(vehTuning), buyDate], function (err, res) {
                      if (err) console.log(err);
...
mp.events.add("server:BoomerangCars:giveCar", boomeranggiveCar);
